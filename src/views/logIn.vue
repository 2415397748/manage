  ...
  <div class="table"
    ...
    <el-form ref="form"
      ...
      <el-form-item label="账号"
                    prop="account"><i class="el-icon-user-solid"
           style="font-size: 1.2rem; line-height: 2.5rem;"></i>
        <el-input v-model="form.account"
                  validate-event
                  placeholder="请输入账号"
                  @keydown.enter.native="submitForm(form.account,form.password)"
                  style="width: 300px;"></el-input>
      </el-form-item>
      <el-form-item label="密码"
                    prop="password"><i class="el-icon-lock"
           style="font-size: 1.2rem; line-height: 2.5rem;"></i>
      ...
      <el-form-item prop="verifyCode"
        ...
        <el-input v-model="form.verifyCode"
                  ...
                  class="identifyinput"
                  style="width:230px;"
                  @keydown.enter.native="submitForm(form.account,form.password)"></el-input>
      </el-form-item>
      <el-form-item>
        <!-- type控制按钮颜色 -->
...
export default {
  ...
  methods: {
    ...
    //注册的判断，是否和数据库账号重合
    resetForm(...age) {
      const account = this.MySQL.filter((items) => items.account === age[0])
      this.$refs.form
        .validate()
        .then((res) => {
          if (account.length > 0) {
